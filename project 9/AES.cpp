...
void datagen()//用于产生符合要求的随机数据
{
	...
	for (int i = 0; i < 10; i++)
	{
		for (int u = 0; u < 4; u++)
		{
			for (int v = 0; v < 4; v++)
			{
				use[i][0][u][v] = rand() % 256;
				if (u == 0 && v == 0)
				{
						use[i][1][u][v] = (use[i][0][u][v] & (~mask)) | (use[i][0][u][v] ^ mask);
				}
				else use[i][1][u][v] = use[i][0][u][v];
			}
		}
	}
	cout << "输入：" << endl;
...
void MC(int i)
{
	...
	for (int x = 1; x < 4; x++)
	{
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				pre[j][k] ^= onlyMC[Ans[i][x][k]][(j - x + 4) % 4];
			}
		}
	}
	for (int j = 0; j < 4; j++)
...
void myAES128(int x, int y)
{
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			if (y == 0)
			{
				use[x + 10][0][i][j] = Ans[2][i][j];
			}
			else use[x + 10][0][i][j] ^= Ans[2][i][j];
		}
	}
}
void myAES128_nomc(int x, int y)
{
	for (int i = 1; i <= 2; i++)
	{
		ShiftRows(i);
...
void myAES128_nosbox(int x, int y)
{
	for (int i = 1; i <= 2; i++)
	{
		ShiftRows(i);
...
int main()
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int k = 0; k < 4; k++)
			{
				cout << hex << (use[i + 10][0][j][k] <= 15 ? "0" : "") << use[i + 10][0][j][k] << " ";
			}cout << endl;
		}
	}
	cout << "删除MC:" << endl;
